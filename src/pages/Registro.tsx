...
import { registrarComoEntrenador } from "../lib/auth";
...
export default function Registro() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setError(null);
    try {
      await registrarComoEntrenador({ nombre, email, password });
      setEnviado(true);
    } catch (err) {
        ...
        {enviado ? (
          <div className="mt-6 space-y-4 text-center">
            <p className="text-sm text-navy-800/80">
              Registro recibido. Un administrador del club tiene que aprobar tu acceso antes de que puedas entrar.
            </p>
          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-4">
            <div>
              <label htmlFor="nombre" className="block text-sm font-medium text-navy-800">
                Nombre
              </label>
              <input
                id="nombre"
